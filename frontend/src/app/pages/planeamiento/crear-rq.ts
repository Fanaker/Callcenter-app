...
export class CrearRq implements OnInit {
  ...
  ngOnInit(): void {
    forkJoin({
      campanias: this.rqService.getCampaniasActivas(),
      puestos: this.rqService.getPuestosActivos()
    }).subscribe({
      next: ({ campanias, puestos }) => {
        this.campaniasActivas = campanias;
        this.puestosActivos = puestos;
        this.cargandoListas = false;
      },
      error: () => {
        this.cargandoListas = false;
        alert('No se pudieron cargar campanias o puestos');
      }
    });
  }
  ...
  crearRQ(): void {
    ...
    this.guardando = true;
    this.rqService.add(rq).subscribe({
      next: () => {
        this.router.navigate(['/planeamiento']);
      },
      error: () => {
        this.guardando = false;
        alert('No se pudo crear el requerimiento');
      }
    });
  }
